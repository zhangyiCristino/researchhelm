...
MODEL_TIERS = ("light", "balanced", "frontier")
MODEL_TIER_RANK = {tier: index for index, tier in enumerate(MODEL_TIERS)}
# Minimum tier per stage; see references/model-routing.md.
MODEL_STAGE_FLOORS = {
    "IDEA_SCOUT": "balanced",
...
def _validate_brief(path: Path, brief: Any, findings: list[Finding]) -> None:
    ...
    _validate_model_selection(path, brief, findings)
    for finding in validate_resume_binding(brief):
        findings.append(Finding(finding.code, str(path), finding.message))


def _validate_model_tiers(path: Path, resources: Any,
                          findings: list[Finding]) -> None:
    """Validate the optional resources.model_tiers mapping."""
    if not isinstance(resources, dict) or "model_tiers" not in resources:
        return
    tiers = resources.get("model_tiers")
    valid = isinstance(tiers, dict) and all(
        key in MODEL_TIERS and isinstance(value, str) and value.strip()
    ...
    if not valid:
        findings.append(
            Finding(
                "model.invalid_tier",
                str(path),
                "model_tiers must map light, balanced, or frontier to "
                "non-empty model identifiers",
            )
        )


def _validate_model_selection(path: Path, brief: Any,
                              findings: list[Finding]) -> None:
    """Validate the optional model_selection record and its stage floor."""
    if isinstance(brief, dict):
        _validate_model_tiers(path, brief.get("resources"), findings)
    if not isinstance(brief, dict) or "model_selection" not in brief:
        return
    selection = brief.get("model_selection")
    ...
    _validate_enum(findings, path, selection, "stage", STAGES)
    _validate_hash(findings, path, selection, "approval_hash")

    for field in ("recommended_tier", "approved_tier"):
        if selection.get(field) not in MODEL_TIERS:
            findings.append(
                Finding(
                    "model.invalid_tier",
                    str(path),
                    f"{field} must be light, balanced, or frontier",
                )
            )

    floor = selection.get("floor")
    if floor is not None and floor not in MODEL_TIERS:
        findings.append(
            Finding(
                "model.invalid_tier",
                str(path),
                "floor must be null, light, balanced, or frontier",
            )
        )

    score = selection.get("score")
    if not isinstance(score, int) or isinstance(score, bool) or not 0 <= score <= 60:
        findings.append(
            Finding(
                "model.invalid_tier",
                str(path),
                "score must be an integer between 0 and 60",
            )
        )

    if not isinstance(selection.get("model"), str) or not selection.get("model"):
        findings.append(
            Finding(
                "model.invalid_tier",
                str(path),
    ...
    approved = selection.get("approved_tier")
    stage = selection.get("stage")
    required = MODEL_STAGE_FLOORS.get(stage)
    if isinstance(floor, str) and floor in MODEL_TIERS:
        if required is None or MODEL_TIER_RANK[floor] > MODEL_TIER_RANK[required]:
            required = floor
    if (
        required
        and approved in MODEL_TIERS
        and MODEL_TIER_RANK[approved] < MODEL_TIER_RANK[required]
    ):
        findings.append(
            Finding(
                "model.floor_violation",
                str(path),
                f"approved_tier is below the {required} floor for this stage",
            )
        )
